...
$$
f(x)=\left\{\begin{array}{ll}{x} & {\text { if } x>0} \\ {a x} & {\text { otherwise }}\end{array}\right.
$$

　　因为 $\alpha \le 1$（用来控制 Leaky 的程度，可以作为超参，也可以用网络自己学习），所以上式可以写成：

$$
{\displaystyle f(x)=\max(x,ax)}
$$

　　这样就可以跟 maxout 网络产生了联系。

![-w534](/img/media/15707787948660.jpg)
