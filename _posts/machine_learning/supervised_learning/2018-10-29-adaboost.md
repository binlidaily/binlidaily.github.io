...
　　Adaboost (**Ada**ptive **Boost**ing) 是一个对“三个臭皮匠顶个诸葛亮”的最好诠释，其利用多个弱学习器的线性组合来达到一个强分类器的效果。在算法中会对每一个样本赋予一个权重，在后续的训练中会提高前一轮被误分类的样本权重，而降低正确分类的样本权重。对于弱学习器的分类任务，结合策略可以利用加权多数表决的方法，对于回归任务可以采用加权平均。
...
　　当所有的基学习器训练好后，我们能得到一个集成的最终学习器，可以看出 Adaboost 是一个加法模型，最终的强学习器是由若干个弱学习器加权平均得到：
...
　　知道所有的弱学习器是如何合作决策后，那么我们现在来搞清楚如何在训练时更新这些弱学习器的权重，以及每一个弱学习器下的样本权重 $D(k)$。
...
　　分类错误率 $e _ { k }$ 只记录那些分错的样本的权重并累加，这里 $\sum_{i=1}^{m} w_{k i}=1$。

　　Adaboost 的残差学习方法是采用前向分步学习算法，我们记第 $k-1$ 轮的强学习器为：
...
　　首先，我们求 $G_k(x)$，可以把 $\alpha$ 看成常数，损失函数可理解为求弱分类器 $G_k(x)$ 使得其误分类样本的数量尽可能的少。
...
**输入**：训练数据集 $T = \left\{ \left( x_1, y_1 \right), \left( x_2, y_2 \right), \cdots, \left( x_N, y_N \right) \right\} $，其中 $x_i \in \mathcal{X} \subseteq R^{n}$, $y_{i} \in \mathcal{Y} = \left\{ +1, -1 \right\}$, $i = 1, 2, \cdots, N $；弱学习算法
...
$$\begin{align*} \\ & D_{m+1}=\left(w_{m+1,1},\cdots,w_{m+1,i},\cdots,w_{m+1,N}\right) \\ & w_{m+1,i} = \dfrac{w_{mi}}{Z_{m}} \exp \left(- \alpha_{m} y_{i} G_{m}\left(x_{i}\right)\right), \\ & \quad \quad = \left\{ \begin{aligned} \ & \dfrac{w_{mi}}{Z_{m}} \exp \left(- \alpha_{m} \right), G_{m}\left(x_{i}\right) = y_{i} \\ & \dfrac{w_{mi}}{Z_{m}} \exp \left( \alpha_{m} \right), G_{m}\left(x_{i}\right) \neq y_{i} \end{aligned} \right. \quad i=1,2,\cdots,N \end{align*}$$
