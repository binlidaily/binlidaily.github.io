...
&=\underset{G}{\operatorname{arg min}}\left\{K+\sum_{i : G\left(x_{i}\right) \neq y_{i}}  w _ { k i } ^ { \prime }\left(\exp\left({\alpha}\right)-\exp\left({-\alpha}\right)\right)\right\} \quad  K=\sum_{i=1}^{n}  w _ { k i } ^ { \prime } \exp\left({-\alpha}\right) \text{, K is a constant}\\ 
