...
**输入**：训练数据集 $T = \text{\{} \left( x_1, y_1 \right), \left( x_2, y_2 \right), \cdots, \left( x_N, y_N \right) \text{\}}$，其中 $x_i \in \mathcal{X} \subseteq R^{n}$, $y_{i} \in \mathcal{Y} = \{ +1, -1 \}$, $i = 1, 2, \cdots, N$；
