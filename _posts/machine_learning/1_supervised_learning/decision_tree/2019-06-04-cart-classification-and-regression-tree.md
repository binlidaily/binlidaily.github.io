...
### 1.1 分类树的生成
　　与 ID3 和 C4.5 不同的是，CART 划分的结果是二叉树。在生成分类树时对每一个属性 $a$ 的每一种取值 $a^v$ 判断“当前属性值是否小于等于 $a^v$”，如此将样本集 $D$ 分成 $D_1$ 和 $D_2$ 两个部分。遍历属性 $a$ 的所有取值 $a^v$，找到能够使得[基尼系数](http://hi-bin.com/2018-09-11-decision-tree/#113-基尼系数gini-index)最小化的属性值。
