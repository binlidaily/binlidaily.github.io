...
## 0. 期望和方差
　　在 Bagging 和 Boosting 框架中，通过计算基学习器的期望和方差我们能得到整体的期望和方差。为了简化模型，假设基学习器期望为 $\mu$，方差为 $\sigma^2$，模型的权重为 $r$，两两模型建的相关系数 $\rho$ 相等。由于 Bagging 和 Boosting 的基学习器都是线性组成的，那么模型总体的期望可以计算为：
...
　　我们可以得到：（这里有些问题，待整理！）
* Booting 采用基于贪心策略的前相加法模型，整体模型的期望由各个基学习器的期望累加二乘，基学习器越多，整体期望越高，对应的准确率越高？
