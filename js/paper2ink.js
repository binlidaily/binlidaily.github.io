(function () {
  ...
  async function vectorize(file, smoothness) {
    var bitmap=await createImageBitmap(file), maxEdge=1800, ratio=Math.min(1,maxEdge/Math.max(bitmap.width,bitmap.height)), w=Math.max(1,Math.round(bitmap.width*ratio)), h=Math.max(1,Math.round(bitmap.height*ratio));
    ...
    var threshold=otsu(normalized),binary=new Uint8Array(w*h);for(i=0;i<binary.length;i++)binary[i]=normalized[i]<threshold?1:0;var closed=removeNoise(morphology(morphology(binary,w,h,true),w,h,false),w,h,2);
    await new Promise(function(resolve){setTimeout(resolve,0);});
    var raw=traceShapes(closed,w,h);if(raw.length>12000)throw new Error("图片背景过于复杂，请先裁剪到笔记区域后重试");var epsilon=.25+(smoothness/100)*1.2,scale=Math.min(1200/w,800/h),offsetX=(1200-w*scale)/2,offsetY=(800-h*scale)/2;
    var contours=raw.map(function(loop){var step=Math.max(1,Math.ceil(loop.length/3000)),sampled=step===1?loop:loop.filter(function(_,index){return index%step===0;});return rdp(sampled.map(function(n){return{x:(n%(w+1))*scale+offsetX,y:Math.floor(n/(w+1))*scale+offsetY};}),Math.max(.3,epsilon*.55)*scale);}).filter(function(cn){return cn.length>=3;});
  ...
  function vectorizeSafely(file, smoothness) {
    ...
    return new Promise(function (resolve, reject) {
      var worker = new Worker("/js/paper2ink-worker.js?v=3");
