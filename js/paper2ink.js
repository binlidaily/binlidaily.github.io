(function () {
  ...
  async function vectorize(file, smoothness) {
    var bitmap=await createImageBitmap(file), maxEdge=2400, ratio=Math.min(1,maxEdge/Math.max(bitmap.width,bitmap.height)), w=Math.max(1,Math.round(bitmap.width*ratio)), h=Math.max(1,Math.round(bitmap.height*ratio));
    ...
    await new Promise(function(resolve){setTimeout(resolve,0);});
    var raw=traceShapes(closed,w,h);if(raw.length>12000)throw new Error("图片背景过于复杂，请先裁剪到笔记区域后重试");var epsilon=.08+(smoothness/100)*.45,scale=Math.min(1200/w,800/h),offsetX=(1200-w*scale)/2,offsetY=(800-h*scale)/2;
  ...
  function vectorizeSafely(file, smoothness) {
    ...
    return new Promise(function (resolve, reject) {
      var worker = new Worker("/js/paper2ink-worker.js?v=4");
